...
const getPlatform = (): string => {
	if (typeof navigator === 'undefined') return 'some platform'
	const _navigator = navigator as any
	const _platform = _navigator?.userAgentData?.platform || navigator?.platform
	return _platform || ''
}
...
export const injectScript = (src: string, id: string, callback: Function) => {
	if (!document)
		throw new Error("Cant't inject script outside browser/document.")
